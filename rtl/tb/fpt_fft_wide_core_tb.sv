...
module fpt_fft_wide_core_tb;
    ...
    logic [LANES-1:0][$clog2(POINTS)-2:0] twiddle_index;
    logic signed [LANES-1:0][TWIDDLE_WIDTH-1:0] twiddle_c;
    logic signed [LANES-1:0][TWIDDLE_WIDTH-1:0] twiddle_c_minus_d;
    logic signed [LANES-1:0][TWIDDLE_WIDTH-1:0] twiddle_c_plus_d;
    logic output_valid;
    ...
    always @* begin
        for (lane = 0; lane < LANES; lane = lane + 1) begin
            twiddle_c[lane] = c_rom[twiddle_index[lane]];
            twiddle_c_minus_d[lane] = c_minus_d_rom[twiddle_index[lane]];
            twiddle_c_plus_d[lane] = c_plus_d_rom[twiddle_index[lane]];
        end
    end
    ...
    initial begin
        ...
        for (frame = 0; frame < FRAMES; frame = frame + 1) begin
            ...
            for (index = 0; index < POINTS; index = index + LANES) begin
                @(negedge clk);
                if (!input_ready)
                    $fatal(1, "wide FFT input backpressure mismatch");
                for (lane = 0; lane < LANES; lane = lane + 1) begin
            ...
            for (index = 0; index < POINTS; index = index + LANES) begin
                #1;
                for (lane = 0; lane < LANES; lane = lane + 1) begin
                    if ($signed(output_real[lane]) !=
                        frame_expected_real[index + lane])
                        $fatal(1,
                            "wide FFT real mismatch frame=%0d index=%0d actual=%0d expected=%0d",
                            frame, index + lane, $signed(output_real[lane]),
                            frame_expected_real[index + lane]);
                    if ($signed(output_imag[lane]) !=
                        frame_expected_imag[index + lane])
                        $fatal(1,
                            "wide FFT imag mismatch frame=%0d index=%0d actual=%0d expected=%0d",
                            frame, index + lane, $signed(output_imag[lane]),
                            frame_expected_imag[index + lane]);
                end
                @(posedge clk);
            end
            #1;
            output_ready = 1'b0;
            if (!done) $fatal(1, "wide FFT done pulse missing");
        end
        $fclose(vector_file);
        $display("PASS fpt_fft_wide_core frames=%0d points=%0d lanes=%0d",
                 FRAMES, POINTS, LANES);
        $finish;
    end
endmodule
