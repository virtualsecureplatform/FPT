...
output_dir=${3:-$repo_root/build/chisel-paper-bitwise-blind-rotate-sample-extract}
domain_dimension=${FPT_BLIND_ROTATE_DIMENSION:-630}

forward=$(realpath "$forward")
inverse=$(realpath "$inverse")
output_dir=$(realpath -m "$output_dir")
output_file=$output_dir/BatchedBlindRotateSampleExtractEngine.sv
...
(cd "$repo_root/chisel" &&
    sbt -J-Xmx"$heap_size" \
        "runMain fpt.EmitPaperBitwiseBatchedBlindRotateSampleExtract $output_dir $forward $inverse $domain_dimension")

if [[ ! -s $output_file ]]; then
    echo "Chisel did not emit $output_file" >&2
    exit 1
fi
if ! rg -q '^module BatchedBlindRotateSampleExtractEngine\(' "$output_file" ||
   ! rg -q '^module SampleExtractIndexZero\(' "$output_file"; then
...
if command -v verilator >/dev/null && [[ ${FPT_SKIP_LINT:-0} != 1 ]]; then
    verilator --lint-only -Wno-fatal \
        --top-module BatchedBlindRotateSampleExtractEngine \
        "$output_file" "$forward" "$inverse"
fi

echo "Emitted paper-shaped sample-extracted bitwise batched Blind Rotate in $output_file"
