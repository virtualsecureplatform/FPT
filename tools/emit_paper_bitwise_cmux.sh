...
output_dir=${3:-$repo_root/build/chisel-paper-bitwise-cmux}

forward=$(realpath "$forward")
inverse=$(realpath "$inverse")
output_dir=$(realpath -m "$output_dir")
output_file=$output_dir/CmuxEngine.sv
...
(cd "$repo_root/chisel" &&
    sbt -J-Xmx"$heap_size" \
        "runMain fpt.EmitPaperBitwiseCmux $output_dir $forward $inverse")
...
echo "Emitted paper-shaped bitwise CMUX in $output_file"
