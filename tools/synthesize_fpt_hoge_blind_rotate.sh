...
fpt_external_product_products=$(rg -c \
    '^  wire \[56:0\] +(ac|bd|ad|bc)(_[0-9]+)? = .* \* .*;' \
    "$fpt_br" || true)
if [[ ! $fpt_forward_products =~ ^[1-9][0-9]*$ || \
      ! $fpt_inverse_products =~ ^[1-9][0-9]*$ || \
      ! $fpt_external_product_products =~ ^[1-9][0-9]*$ || \
      $((fpt_external_product_products % 2)) -ne 0 ]]; then
    echo "Unexpected FPT split-product structure: forward=$fpt_forward_products inverse=$fpt_inverse_products external=$fpt_external_product_products" >&2
    exit 1
fi
# CIRCT represents each 30x27-bit SInt product as a 57x57-bit unsigned modular
# product over sign-extended operands. The complete flattened Yosys map merges
# the two buffer copies, but each of the remaining logical products occupies
# nine DSP48E2s. A separately mapped External Product therefore uses exactly
# (2048 / 2) * 9 = 9216 DSPs. Two additional DSPs are in wrapper glue.
fpt_external_product_dsps=$((fpt_external_product_products / 2 * 9))
