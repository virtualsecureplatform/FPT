...
output_dir=${3:-$repo_root/build/chisel-paper-buffered-blind-rotate-sample-extract}
...
output_file=$output_dir/BufferedBatchedBlindRotateSampleExtractEngine.sv
...
(cd "$repo_root/chisel" &&
    sbt -J-Xmx"$heap_size" \
        "runMain fpt.EmitPaperBufferedBitwiseBatchedBlindRotateSampleExtract $output_dir $forward $inverse $domain_dimension $arithmetic_profile")

if [[ ! -s $output_file ]]; then
    echo "Chisel did not emit $output_file" >&2
    exit 1
fi
for module in BufferedBatchedBlindRotateSampleExtractEngine \
...
if command -v verilator >/dev/null && [[ ${FPT_SKIP_LINT:-0} != 1 ]]; then
    verilator --lint-only -Wno-fatal \
        --top-module BufferedBatchedBlindRotateSampleExtractEngine \
        "$output_file" "$forward" "$inverse"
fi

echo "Emitted paper-shaped buffered Blind Rotate in $output_file"
