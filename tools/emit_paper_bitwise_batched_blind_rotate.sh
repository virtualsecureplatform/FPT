...
output_dir=${3:-$repo_root/build/chisel-paper-bitwise-blind-rotate}
...
(cd "$repo_root/chisel" &&
    sbt -J-Xmx"$heap_size" \
        "runMain fpt.EmitPaperBitwiseBatchedBlindRotate $output_dir $forward $inverse $domain_dimension")
...
echo "Emitted paper-shaped bitwise batched Blind Rotate in $output_file"
