...
proc get_cells {args} {
  ...
  if {![regexp {^NAME =~ (\S+) && REF_NAME =~ (\S+) && REF_NAME !~ \*DistributedChild\* && IS_PRIMITIVE == 0$} $filter -> pattern ref]} {
    error "missing local-bank type/primitive filter: $filter"
  }
  set result {}
  foreach {name type primitive} $::fixtures {
    if {!$primitive && [string match $pattern $name] && [string match $ref $type] && ![string match *DistributedChild* $type]} {lappend result $name}
  }
  return $result
}
set fixtures {
  ...
  top/rotating/finalBankTiles_1_0_0/subtiles[0].bank FptFinalAccumulatorLocalDistributedChild 0
