...
int main(int argc, char **argv)
{
    ...
    std::uint64_t key_load_beat = 0;

    const auto tick = [&] {
        dut.clock = 0;
        dut.eval();

        // Stream the complete zero-valued bootstrapping key. The first two
        // coefficients fill the ping-pong cache during context initialization;
        // subsequent coefficients load as Blind Rotate retires each bank.
        dut.io_keyLoadStart = 0;
        dut.io_keyLoadValid = 0;
        bool key_load_start_fire = false;
        bool key_load_fire = false;
        if (!dut.reset && next_key_load_index < dimension) {
            dut.io_keyLoadIndex = next_key_load_index;
            if (key_load_active) {
                dut.io_keyLoadValid = 1;
                if (key_load_beat ==
                        key_load_beats_per_coefficient - 1 &&
                    next_key_load_index + 1 < dimension) {
                    dut.io_keyLoadIndex = next_key_load_index + 1;
                    dut.eval();
                    if (dut.io_keyLoadStartReady)
                        dut.io_keyLoadStart = 1;
                }
            } else if (dut.io_keyLoadStartReady) {
                dut.io_keyLoadStart = 1;
            }
            dut.eval();
            key_load_start_fire =
                dut.io_keyLoadStart && dut.io_keyLoadStartReady;
            key_load_fire = dut.io_keyLoadValid && dut.io_keyLoadReady;
        }
        ...
        if (!dut.reset) {
            if (dut.io_inputValid && dut.io_inputReady)
                ++input_coefficients;
            if (key_load_start_fire) ++key_load_starts;
            if (key_load_fire) ++key_load_beats;
            if (dut.io_keyLoadDone) {
                if (dut.io_keyLoadDoneIndex != key_load_completions)
            ...
            if (dut.io_keyValid && dut.io_keyFirst) {
                ...
                const std::uint64_t expected_context =
                    key_transactions % contexts;
                if (dut.io_keyIndex != expected_index ||
                    dut.io_keyContext != expected_context)
                    throw std::runtime_error(
                        "FPT issued bootstrapping-key transactions out of order");
        ...
        ++cycles;
        if (key_load_fire) {
            if (key_load_beat == key_load_beats_per_coefficient - 1) {
                key_load_active = key_load_start_fire;
                key_load_beat = 0;
                ++next_key_load_index;
            } else {
                ++key_load_beat;
            }
        } else if (key_load_start_fire) {
            key_load_active = true;
            key_load_beat = 0;
        }
        return done;
    };
    ...
    if (key_load_starts != dimension ||
        key_load_completions != dimension ||
    ...
    std::cout << "fpt_buffered_blind_rotate_batch_cycles="
              << batch_cycles << '\n'
              << std::fixed << std::setprecision(1)
              << "fpt_buffered_blind_rotate_cycles_per_result="
              << static_cast<double>(batch_cycles) /
                     static_cast<double>(contexts)
              << '\n'
              << "fpt_buffered_blind_rotate_input_phase_cycles="
              << input_phase_cycles << '\n'
              << "fpt_buffered_blind_rotate_compute_phase_cycles="
              << compute_phase_cycles << '\n'
              << "fpt_buffered_blind_rotate_drain_tail_cycles="
              << drain_tail_cycles << '\n'
